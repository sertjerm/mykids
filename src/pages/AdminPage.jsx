...
import { Plus, Edit, Trash2, Save, X, Home, Users, CheckCircle, XCircle, Gift } from 'lucide-react';
...
const AdminPage = () => {
  ...
  useEffect(() => {
    ...
    if (savedBadBehaviors) {
      ...
      localStorage.setItem('badBehaviors', JSON.stringify(DEFAULT_BAD_BEHAVIORS));
    }
  ...
  const saveItem = () => {
    ...
    switch (item.type) {
      ...
      case 'badBehaviors':
        if (editingItem) {
          setBadBehaviors(prev => prev.map(b => b.id === id ? itemData : b));
        } else {
      ...
      <div className="container mx-auto p-4">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            <a href="/" className="flex items-center gap-2 px-4 py-2 bg-white rounded-lg shadow hover:shadow-md transition-shadow">
              <Home size={18} />
              กลับหน้าหลัก
            </a>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6">
            ...
            {activeTab === 'children' && (
              <div>
                ...
                <div className="space-y-3">
                  {children.map(child => (
                    <div key={child.id} className="p-4 rounded-lg border-2 border-gray-200" style={{ backgroundColor: child.bgColor + '40' }}>
                      {editingItem && editingItem.id === child.id ? (
                        <div className="space-y-3">
                          ...
                          <input
                            ...
                            onChange={(e) => updateEditingField('age', parseInt(e.target.value) || '')}
                            placeholder="อายุ"
                            className="w-full p-2 border rounded-lg"
                          />
                          <div>
                            <label className="block text-sm font-medium mb-2">เลือกอิโมจิ:</label>
                            <div className="grid grid-cols-8 gap-2">
                              {EMOJI_OPTIONS.map(emoji => (
                                <button
                                  key={emoji}
                                  onClick={() => updateEditingField('emoji', emoji)}
                                  className={`p-2 text-xl rounded border-2 hover:bg-gray-100 ${
                                    currentItem.emoji === emoji ? 'border-blue-500 bg-blue-100' : 'border-gray-200'
                                  }`}
                                >
                                  {emoji}
                                </button>
                          ...
                          <div>
                            <label className="block text-sm font-medium mb-2">เลือกสี:</label>
                            <div className="grid grid-cols-4 gap-2">
                              {COLOR_OPTIONS.map(color => (
                                <button
                                  key={color.value}
                                  onClick={() => updateEditingField('bgColor', color.value)}
                                  className={`p-3 rounded border-2 hover:scale-105 transition-transform ${
                                    currentItem.bgColor === color.value ? 'border-blue-500 ring-2 ring-blue-200' : 'border-gray-200'
                                  }`}
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-gray-600">{child.age ? `${child.age} ขวบ` : 'ไม่ระบุอายุ'}</p>
                            </div>
                          </div>
                          <div className="flex gap-2">
                            <button
                            ...
                            <button
                              ...
                              disabled={isAddingNew || editingItem || children.length === 1}
                            >
                              <Trash2 size={16} />
                            </button>
                          </div>
                ...
                <div className="mt-4 p-3 bg-gray-100 rounded-lg">
                  <p className="text-sm text-gray-600">
                    💡 <strong>คำแนะนำ:</strong> ต้องมีลูกอย่างน้อย 1 คนในระบบ • การลบลูกจะลบข้อมูลทั้งหมด • สีและอิโมจิจะแสดงในหน้าหลัก
                  </p>
                </div>
              </div>
            )}
            ...
            {activeTab === 'behaviors' && (
              <div>
                <div className="flex justify-between items-center mb-4">
                  <h2 className="text-xl font-bold">✅ จัดการงานและกิจกรรมดี</h2>
                  <button
                ...
                <div className="space-y-3">
                  {behaviors.map(behavior => (
                    <div key={behavior.id} className="p-4 rounded-lg border-2 border-gray-200" style={{ backgroundColor: behavior.color + '60' }}>
                      {editingItem && editingItem.id === behavior.id ? (
                        <div className="space-y-3">
                          ...
                          <input
                            ...
                            onChange={(e) => updateEditingField('points', parseInt(e.target.value) || 0)}
                          ...
                          <div>
                            ...
                            <div className="grid grid-cols-4 gap-2">
                              {COLOR_OPTIONS.map(color => (
                                <button
                                  key={color.value}
                                  onClick={() => updateEditingField('color', color.value)}
                                  className={`p-3 rounded border-2 hover:scale-105 transition-transform ${
                                    currentItem.color === color.value ? 'border-blue-500 ring-2 ring-blue-200' : 'border-gray-200'
                                  }`}
                                  style={{ backgroundColor: color.value }}
                                >
                                  <span className="text-xs font-medium">{color.name}</span>
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            <span className="font-medium text-lg">{behavior.name}</span>
                          </div>
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              onClick={() => deleteItem(behavior.id, 'behaviors')}
                              className="p-2 text-red-600 hover:bg-red-100 rounded"
                              disabled={isAddingNew || editingItem}
                            >
                              <Trash2 size={16} />
                    ...
                    <div className="p-4 rounded-lg border-2 border-dashed border-green-300 bg-green-50">
                      <div className="space-y-3">
                        ...
                        <div className="mt-3 p-3 bg-blue-50 rounded-lg">
                          <p className="text-sm text-blue-700 font-medium mb-2">💡 แนะนำงานยอดนิยม:</p>
                          <div className="grid grid-cols-1 gap-1 text-xs text-blue-600">
                            <div>• 🦷 แปรงฟัน (3-5 คะแนน) • 🧸 เก็บของเล่น (2-3 คะแนน)</div>
                            <div>• 📚 อ่านหนังสือ (4-6 คะแนน) • 📝 ทำการบ้าน (8-10 คะแนน)</div>
                            <div>• 🥗 ทานผัก (4-6 คะแนน) • 🏃 ออกกำลังกาย (5-7 คะแนน)</div>
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <div className="mt-4 p-3 bg-gray-100 rounded-lg">
                  <p className="text-sm text-gray-600">
                    💡 <strong>คำแนะนำ:</strong> งานง่าย 1-3 คะแนน • งานปานกลาง 4-6 คะแนน • งานยาก 7+ คะแนน
                  </p>
                </div>
              </div>
            )}
            ...
            {activeTab === 'badBehaviors' && (
              <div>
                      ...
                      {editingItem && editingItem.id === behavior.id ? (
                        <div className="space-y-3">
                          ...
                          <input
                            ...
                            onChange={(e) => updateEditingField('penalty', -(parseInt(e.target.value) || 0))}
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              onClick={() => startEdit(behavior, 'badBehaviors')}
                              className="p-2 text-blue-600 hover:bg-blue-100 rounded"
                              disabled={isAddingNew || editingItem}
                            >
                              <Edit size={16} />
                            ...
                            <button
                              onClick={() => deleteItem(behavior.id, 'badBehaviors')}
                              className="p-2 text-red-600 hover:bg-red-100 rounded"
                              disabled={isAddingNew || editingItem}
                            >
                              <Trash2 size={16} />
                    ...
                    <div className="p-4 rounded-lg border-2 border-dashed border-red-300 bg-red-50">
                      <div className="space-y-3">
                        ...
                        <div className="mt-3 p-3 bg-orange-50 rounded-lg">
                          <p className="text-sm text-orange-700 font-medium mb-2">⚠️ พฤติกรรมไม่ดีที่พบบ่อย:</p>
                          <div className="grid grid-cols-1 gap-1 text-xs text-orange-600">
                            <div>• 😤 พูดหยาบ (-2 ถึง -4) • 🤥 โกหก (-4 ถึง -6)</div>
                            <div>• 😭 งอแง ร้องไห้ (-1 ถึง -3) • 🤜 ทำร้ายพี่น้อง (-6 ถึง -10)</div>
                            <div>• 📱 เล่นมือถือนานเกิน (-3 ถึง -5) • 🚫 ไม่ฟังคำสั่ง (-4 ถึง -8)</div>
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <div className="mt-4 p-3 bg-gray-100 rounded-lg">
                  <p className="text-sm text-gray-600">
                    💡 <strong>คำแนะนำ:</strong> เล็กน้อย 1-3 คะแนน • ปานกลาง 4-6 คะแนน • ร้ายแรง 7+ คะแนน
                  </p>
                </div>
              </div>
            )}
            ...
            {activeTab === 'rewards' && (
              <div>
                ...
                <div className="space-y-3">
                  {rewards.map(reward => (
                    <div key={reward.id} className="p-4 rounded-lg border-2 border-gray-200 bg-gradient-to-r from-purple-100 to-pink-100">
                      {editingItem && editingItem.id === reward.id ? (
                        <div className="space-y-3">
                          ...
                          <input
                            ...
                            onChange={(e) => updateEditingField('cost', parseInt(e.target.value) || 0)}
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="font-medium text-lg">{reward.name}</span>
                          </div>
                          <div className="flex items-center gap-2">
                    ...
                    <div className="p-4 rounded-lg border-2 border-dashed border-purple-300 bg-purple-50">
                      <div className="space-y-3">
                        ...
                        <div className="mt-3 p-3 bg-yellow-50 rounded-lg">
                          <p className="text-sm text-yellow-700 font-medium mb-2">🎁 ไอเดียรางวัล:</p>
                          <div className="grid grid-cols-1 gap-1 text-xs text-yellow-600">
                            <div>• 🍦 ขนมหวาน (5-10 คะแนน) • 🎮 เล่นเกม 30 นาที (15-20)</div>
                            <div>• 🎬 ดูหนัง (20-25) • 🛍️ ซื้อของเล่น (30-50)</div>
                ...
                <div className="mt-4 p-3 bg-gray-100 rounded-lg">
                  <p className="text-sm text-gray-600">
                    💡 <strong>คำแนะนำ:</strong> รางวัลเล็ก 5-15 คะแนน • รางวัลกลาง 20-50 คะแนน • รางวัลใหญ่ 60+ คะแนน
                  </p>
                </div>
              </div>
            )}
          ...
          <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="bg-white rounded-lg p-4 text-center">
              <div className="text-2xl font-bold text-green-600">{behaviors.length}</div>
              <div className="text-sm text-gray-600">งานดี</div>
            </div>
            <div className="bg-white rounded-lg p-4 text-center">
              <div className="text-2xl font-bold text-red-600">{badBehaviors.length}</div>
