...
const HomePage = () => {
  ...
  useEffect(() => {
    const loadData = () => {
      ...
      const savedBehaviors = JSON.parse(
        localStorage.getItem("behaviors") || "null"
      );
      const savedBadBehaviors = JSON.parse(
        localStorage.getItem("badBehaviors") || "null"
      );
      const savedRewards = JSON.parse(
        localStorage.getItem("rewards") || "null"
      );
      const savedProgress = JSON.parse(
        localStorage.getItem("childrenProgress") || "{}"
      );
      const savedBadBehaviorCounts = JSON.parse(
        localStorage.getItem("childrenBadBehaviorCounts") || "{}"
      );
      const savedUsedPoints = JSON.parse(
        localStorage.getItem("childrenUsedPoints") || "{}"
      );
      ...
      if (!savedChildren)
        localStorage.setItem(
          "children",
          JSON.stringify(DEFAULT_CONFIG.children)
        );
      if (!savedBehaviors)
        localStorage.setItem(
          "behaviors",
          JSON.stringify(DEFAULT_CONFIG.behaviors)
        );
      if (!savedBadBehaviors)
        localStorage.setItem(
          "badBehaviors",
          JSON.stringify(DEFAULT_CONFIG.badBehaviors)
        );
      if (!savedRewards)
        localStorage.setItem("rewards", JSON.stringify(DEFAULT_CONFIG.rewards));
  ...
  useEffect(() => {
    if (Object.keys(childrenProgress).length > 0) {
      localStorage.setItem(
        "childrenProgress",
        JSON.stringify(childrenProgress)
      );
    }
  }, [childrenProgress]);
  ...
  useEffect(() => {
    if (Object.keys(childrenBadBehaviorCounts).length > 0) {
      localStorage.setItem(
        ...
        JSON.stringify(childrenBadBehaviorCounts)
      );
    }
  }, [childrenBadBehaviorCounts]);
  ...
  useEffect(() => {
    if (Object.keys(childrenUsedPoints).length > 0) {
      localStorage.setItem(
        "childrenUsedPoints",
        JSON.stringify(childrenUsedPoints)
      );
    }
  }, [childrenUsedPoints]);
  ...
  const resetChildDay = (childId) => {
    if (
      !confirm(
        "ต้องการเริ่มวันใหม่ใช่ไหม? (จะรีเซ็ตความคืบหน้างานและพฤติกรรมทั้งหมด แต่รางวัลที่แลกไปแล้วจะยังคงอยู่)"
      )
    ) {
      return;
    }
  ...
  const resetAllChildren = () => {
    if (
      !confirm(
        "ต้องการเริ่มวันใหม่สำหรับทุกคนใช่ไหม? (รีเซ็ตความคืบหน้าและพฤติกรรม แต่คะแนนรางวัลที่ใช้ไปจะยังคงอยู่)"
      )
    ) {
      return;
    }
  ...
  const resetAllRewards = (childId) => {
    if (
      !confirm(
        "ต้องการรีเซ็ตคะแนนรางวัลใช่ไหม? (จะทำให้คะแนนที่ใช้แลกรางวัลกลับมาเป็น 0)"
      )
    ) {
      return;
    }
        ...
        <div className="max-w-md mx-auto text-center">
          <div className="bg-white rounded-2xl shadow-lg p-8">
            ...
            <h1 className="text-2xl font-bold text-gray-800 mb-4">
              ยินดีต้อนรับสู่ MyKids!
            </h1>
            <p className="text-gray-600 mb-6">
              ยังไม่มีข้อมูลในระบบ
              กรุณาไปยังหน้าจัดการเพื่อเพิ่มข้อมูลลูกและกิจกรรมต่างๆ
            </p>
            <a
              href="/mykids/?admin"
      ...
      <div className="container mx-auto p-4">
        <div className="max-w-md mx-auto">
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            <a
              href="/mykids/?admin" // แก้ไข URL ให้ใช้ basepath
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div className="text-center">
                    <div className="text-2xl font-bold text-blue-600">
                      {children.reduce(
                        (sum, child) => sum + calculateTotalPoints(child.id),
                        0
                      )}
                    </div>
                  ...
                  <div className="text-center">
                    <div className="text-2xl font-bold text-green-600">
                      {Math.round(
                        children.reduce(
                          (sum, child) => sum + getProgressPercentage(child.id),
                          0
                        ) / children.length
                      )}
                      %
                  ...
                  {children.map((child) => {
                    ...
                    return (
                      <div
                        ...
                        style={{ backgroundColor: child.bgColor + "30" }}
                      >
                        ...
                        <div className="text-xs text-gray-600">
                          งานเสร็จแล้ว: {completed}/{behaviors.length} งาน (
                          {progress}%)
                        </div>
                      </div>
                    );
            ...
            currentChild && (
              <div className="space-y-6">
                ...
                <div
                  ...
                  style={{ backgroundColor: currentChild.bgColor + "20" }}
                >
                  <div className="text-center">
                    ...
                    {(childrenUsedPoints[selectedChild] || 0) > 0 && (
                      <div className="text-sm text-gray-500 mt-1">
                        (ใช้แลกรางวัลไป {childrenUsedPoints[selectedChild]}{" "}
                        คะแนน)
                      </div>
                    )}
                    <div className="text-sm text-gray-600 mt-1">
                      งานเสร็จ: {getCompletedTasksCount(selectedChild)}/
                      {behaviors.length}({getProgressPercentage(selectedChild)}
                      %)
                    </div>
                  </div>
                </div>
                          ...
                          <div key={category} className="mb-4">
                            ...
                            <div className="space-y-2">
                              {categoryBehaviors.map((behavior) => {
                                const isCompleted =
                                  currentProgress[behavior.id] || false;
                                return (
                                  <button
                                    key={behavior.id}
                                    onClick={() =>
                            ...
                            <div key={severity} className="mb-4">
                              ...
                              <div className="space-y-2">
                                {severityBehaviors.map((behavior) => {
                                  const count = getBadBehaviorCount(
                                    selectedChild,
                                    behavior.id
                                  );
                                  return (
                                    <div
                                      ...
                                      style={{
                                        backgroundColor: behavior.color + "40",
                                      }}
                                    >
                                      <div className="flex items-center justify-between">
                                        <span className="font-medium text-gray-800 text-sm">
                              ...
                              {categoryRewards.map((reward) => {
                                ...
                                return (
                                  <button
                                    key={reward.id}
                                    onClick={() =>
                                      redeemReward(selectedChild, reward.id)
                                    }
                                    disabled={!canAfford}
                                    className={`w-full p-3 rounded-xl transition-all transform hover:scale-[1.02] ${
                                      canAfford
                    ...
                    {((activeTab === "behaviors" && behaviors.length === 0) ||
                      ...
                      <div className="text-center py-8 text-gray-500">
                        ...
                        <p className="text-sm mt-1">
                          ไปยังหน้าจัดการเพื่อเพิ่มรายการใหม่
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  {(childrenUsedPoints[selectedChild] || 0) > 0 && (
                    ...
                    >
                      <Star size={16} />
                      รีเซ็ตคะแนนรางวัล ({
                        childrenUsedPoints[selectedChild]
                      }{" "}
                      คะแนน)
                    </button>
                  )}
                ...
                <div className="bg-white rounded-2xl shadow-lg p-6">
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-4 mb-2">
                    <div
                      ...
                      style={{
                        width: `${getProgressPercentage(selectedChild)}%`,
                      }}
                    ></div>
                  </div>
